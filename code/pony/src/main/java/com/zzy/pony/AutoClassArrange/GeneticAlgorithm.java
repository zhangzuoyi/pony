...
//个体适应度函数定义为 如下规则对于每个班满足得1分，不满足则得0分，然后在算一个个体中所有班级的算术平均值
	...
	private void mutation() {
		...
		map = GAUtil.getClassTeacherSubjectweekArrange(DNA.getInstance().getTeacherSubjectweekArrange(),DNA.getInstance().getArrangeRotationMap(),DNA.getInstance().getArrangeCombineMap());
	...
	private void setChromosomeScore(Chromosome chromosome) {
		...
		chromosome.setScore(scoreRuleSeven);
