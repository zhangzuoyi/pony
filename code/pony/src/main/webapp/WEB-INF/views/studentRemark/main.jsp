		...
		<table id="my-datagrid-2" class="easyui-datagrid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'schoolClass',width:100,formatter:operationFormatter">操作</th>
				</tr>
			</thead>
		</table>
	</div>
		...
		<form id="my-form-2" method="post">
			...
			<table>
				...
				<tr>
					...
					<td><textarea rows="5" name="remark" style="width:300px;"></textarea></td>
				</tr>
			</table>
		</form>
	</div>
	...
	<script type="text/javascript">
		function add() {
			$('#my-form-2').form('submit', {
				url : "<s:url value='/studentRemark/add' />",
		...
		function operationFormatter(value, rec, index) {
			return '<a href="javascript:void(0)" class="easyui-linkbutton" onclick="viewRemark(\''
					+ index
					+ '\')" style="width:80px">评语查看</a>'
					+ '<a href="javascript:void(0)" class="easyui-linkbutton" onclick="addRemark(\''
		...
		function viewRemark(index) {
			var data = $('#my-datagrid-2').datagrid('getData').rows;
			var rec = data[index];
			var url = "<s:url value='/studentRemark/studentMain' />" + "?studentId=" + rec.studentId;
			window.parent.addTab("评价查看", url, "icon-users", true);
		}
